...
CREATE TABLE IF NOT EXISTS OPENAQ.BRONZE.LOCATION_SNAPSHOTS (
    LOAD_ID VARCHAR NOT NULL,
    SNAPSHOT_AT TIMESTAMP_TZ NOT NULL,
    RAW_LOCATION VARIANT NOT NULL,
    LOADED_AT TIMESTAMP_TZ NOT NULL DEFAULT CURRENT_TIMESTAMP()
);

COMMENT ON COLUMN OPENAQ.BRONZE.LOCATION_SNAPSHOTS.SNAPSHOT_AT IS
    'End of the Airflow interval whose location discovery response is stored.';

CREATE TRANSIENT TABLE IF NOT EXISTS OPENAQ.BRONZE.LOCATION_STAGING (
    LOAD_ID VARCHAR NOT NULL,
    RAW_LOCATION VARIANT NOT NULL,
    STAGED_AT TIMESTAMP_TZ NOT NULL DEFAULT CURRENT_TIMESTAMP()
);

COMMENT ON COLUMN OPENAQ.BRONZE.LOCATION_STAGING.LOAD_ID IS
    'Separates concurrent or retried location snapshots in the shared staging table.';
...
BEGIN
    ...
    DELETE FROM OPENAQ.BRONZE.LOCATION_SNAPSHOTS
    WHERE LOAD_ID = :LOAD_ID;

    INSERT INTO OPENAQ.BRONZE.LOCATION_SNAPSHOTS (
        LOAD_ID,
        SNAPSHOT_AT,
        RAW_LOCATION
    )
    SELECT
        ...
        :REFRESH_TO,
        RAW_LOCATION
    FROM OPENAQ.BRONZE.LOCATION_STAGING
    WHERE LOAD_ID = :LOAD_ID;
